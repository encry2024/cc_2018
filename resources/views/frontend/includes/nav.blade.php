<nav class="navbar navbar-expand-lg navbar-light bg-vs-blue mb-4">
    <a href="{{ route('frontend.index') }}" class="navbar-brand text-white">{{ app_name() }}</a>
    ...
    <div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
        <ul class="navbar-nav">
            ...
            @guest
                <li class="nav-item"><a href="{{route('frontend.auth.login')}}" class="nav-link text-white {{ active_class(Active::checkRoute('frontend.auth.login')) }}">{{ __('navs.frontend.login') }}</a></li>

                @if (config('access.registration'))
                    <li class="nav-item"><a href="{{route('frontend.auth.register')}}" class="nav-link text-white {{ active_class(Active::checkRoute('frontend.auth.register')) }}">{{ __('navs.frontend.register') }}</a></li>
                @endif
            @else
